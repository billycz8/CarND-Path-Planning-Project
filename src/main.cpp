      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for (int i = 0; i < sensor_fusion.size(); i++) {
            ...
            // Find the object lane
            int OUTSIDE = -42;
            int object_lane = OUTSIDE;
            for (int cur_lane = 0; cur_lane < 3; cur_lane++) {
              if (d < (2+4*cur_lane+2) && d > (2+4*cur_lane-2)) {
                object_lane = cur_lane;
              }
            }
            if (object_lane == OUTSIDE) {
              continue;
            }

            // check lane occupancy of current object
            // default case in front
            if (lane == object_lane) {
              ...
              // case other lane side by side
              if (abs(car_s - check_car_s) < 25) {
                lane_occupied[object_lane] = true;
              } 
              // case it is behind and speed is greater
              else if (((car_s - check_car_s) < 34) && ((car_s - check_car_s) > 0) && (check_speed > 1.1 * car_speed)) {
                lane_occupied[object_lane] = true;
              }
              // case it is in front and the speed is slowing
              else if (((check_car_s - car_s) < 34) && ((check_car_s - car_s) > 0) && (check_speed * 1.1 < car_speed)) {
                lane_occupied[object_lane] = true;
              }
          ...
          if (too_close) {
            // change lane mechanics
            // change left
            if ((lane - 1 >= 0) && (!lane_occupied[lane - 1])) {
              lane = lane - 1;
              if (ref_vel < 49.5) {
                ref_vel += .224;
              }
            ...
            // change right
            else if ((lane + 1 <= 2) && (!lane_occupied[lane + 1])) {
              lane = lane + 1;
              if (ref_vel < 49.5) {
                ref_vel += .224;
              }
          ...
          // if middle lane opened, we go to middle lane so that we have more possibilities
          else if ((!lane_occupied[1])) {
              lane = 1;
              if (ref_vel < 49.5) {
                ref_vel += .224;
              }
